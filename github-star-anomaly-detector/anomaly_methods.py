...
class DetectionConfig:
    ...
    ensemble_min_votes: int = 2  # Minimum methods that must agree for combined detection
...
class MovingAverageDetector(AnomalyDetector):
    ...
    def detect(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        result = df.with_columns([
            pl.col('new_stars').rolling_mean(window_size=self.config.moving_avg_window).alias('moving_avg'),
            pl.col('new_stars').rolling_std(window_size=self.config.moving_avg_window).alias('moving_std')
        ])
        ...
        confidence = np.minimum(ma_deviation_abs / (self.config.moving_avg_threshold * 2), 1.0)

        result = result.with_columns([
            (pl.col('ma_deviation').abs() > self.config.moving_avg_threshold).alias('is_anomaly'),
            pl.Series("confidence", confidence)
        ])
...
class RateChangeDetector(AnomalyDetector):
    ...
    def detect(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Calculate confidence
        confidence = np.minimum(rate_z_scores / (self.config.rate_change_threshold * 2), 1.0)

        result = result.with_columns([
            pl.Series("rate_z_score", rate_z_scores),
            pl.Series("is_anomaly", rate_z_scores > self.config.rate_change_threshold),
            pl.Series("confidence", confidence)
        ])
...
class EnsembleDetector(AnomalyDetector):
    ...
    def detect(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        for i, detector in enumerate(self.detectors):
            ...
            result = result.with_columns([
                detected[f'is_anomaly'].alias(f'is_anomaly_{detector_name}'),
        ...
        result = result.with_columns([
            (pl.col('vote_count') >= self.config.ensemble_min_votes).alias('is_anomaly'),
...
def get_detector(method: str, config: Optional[DetectionConfig] = None) -> AnomalyDetector:
    """
